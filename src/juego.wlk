...
//LADRILLOS
		...
		posicionNiveles.addAll([ new Position(9, 10), new Position(10,10), new Position(11,10), new Position(12,10), new Position(13,10), new Position(14,10), new Position(15,10), new Position(16,10) ])
...
//const pisonivel0 = new Ladrillo()
		...
		var malo1 = new Malo(imagen = "malo2.png", position = game.at(23, 13), cantidadpasosIzquierda = 6, cantidadpasosDerecha = 6)
			// metodos q se usan mas abajo
		var seMueven = [ malo, malo1 ]
		var personajesConGravedad = []
		malo.cantidadPasos()
...
//LADRILLOS
		...
		posicionNiveles.addAll([ new Position(10, 9), new Position(10,10)])
		...
		//piso que se mueve
		var posicionDelPiso=[]
		posicionDelPiso.addAll([new PisoEnMovimiento(position=new Position(11,12)),new PisoEnMovimiento(position=new Position(12,12)),new PisoEnMovimiento(position=new Position(13,12)),new PisoEnMovimiento(position=new Position(14,12)),new PisoEnMovimiento(position=new Position(15,12))])
		posicionDelPiso.forEach{ p => self.dibujar(p)}
		posicionDelPiso.forEach{ p => p.reseteoMovimiento()}
		game.onTick(100, "piso en movimiento", {posicionDelPiso.forEach{ p => p.movimiento()}})  // movimiento del piso
		personajesConGravedad.addAll([ conejo, malo1, malo, caja, caja1 ])
			// COLISIONES
		game.whenCollideDo(conejo, { algo => algo.chocaCon(conejo)})
		game.whenCollideDo(caja1, { algo => algo.chocaCon(caja)})
